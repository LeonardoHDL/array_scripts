...
path_to_python_script=/mnt/Guanina/cvan/data/Keloids_F2/Analysis/leo_analysis/Array_Analysis/array_scripts/adding_variant_info.py


#it requires 5 args:
#1. results of assoc study
...
#3. path to the output file which is a csv file
#4. path to the second output file with the critical p values 
#5 path to the top 10 most significant snps
topten=${outdirectory}${todays_date}_Assoc_results/${todays_date}_top10.csv
module load python38/3.8.3
python3 ${path_to_python_script} ${results_logistic_no_covars} ${output_for_freq_count} ${SNPs_and_freqs} ${file_for_critical_p_vals} ${topten}
